...
(define-public limsn
             (let ((commit "ecaf351c16c505d5444051d32c134c158763ef14");;anchor1
        (revision "2"))
  ...
  (package
    ...
    (source (origin
           (method git-fetch)
                ...
                (sha256 
             (base32 "0y6fs9p1wyk4dpq9gbas8xlyzjhpz8qcqhi90b0yam7nxingkq3l"))));;anchor2
    ...
    (source (origin
		  ...
		  (substitute* "artanis/env.scm"
		    (("  \\(let \\(\\(tmp \\(format #f \"~a/tmp\" \\(proper-toplevel\\)\\)\\)\\)")
		     "  \(let \(\(tmp \(format #f \"/tmp/~a\" \(current-appname))))")  
		    )
		  ;; (substitute* "artanis/env.scm"
		  ;; 	       (("  \\(let \\(\\(tmp \\(format #f \"~a/tmp\" \\(proper-toplevel\\)\\)\\)\\)")
		  ;; 		"  \(let \(\(tmp  \"$HOME/tmp\"))" )  
